    ...
    try {
        ...
        const { docs, total, pages } = await db.product.paginate({
            attributes: [
                ...
                "name",
                "description",
                [sequelize.fn("CONCAT", `${getOriginUrl(req)}/api/products/`, sequelize.col("Product.id")), "detail"]
            ],
            include: [{
                model: db.category,
                as: 'Category',
        ...
        return res.status(200).json({
            ...
            countByCategory,
            products: docs,
            next: offset < pages ? `${getOriginUrl(req)}/api/products/?offset=${offset + 1}` : (pages === 1 ? "-" : "Last page"),
            previous: offset > 1 ? `${getOriginUrl(req)}/api/products/?offset=${offset - 1}` : (pages === 1 ? "-" : "First page")
        });
    } catch (error) {
        res.status(500).json({
