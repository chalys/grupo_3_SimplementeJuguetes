window.addEventListener("load", async () => {
  ...
  const storedProvince = selectProvince.getAttribute('data-stored-value');
  const storedCity = selectCity.getAttribute('data-stored-value');

  if (storedProvince) {
    selectProvince.value = storedProvince;
    const event = new Event('change');
    selectProvince.dispatchEvent(event);

    if (storedCity) {
      selectCity.setAttribute('data-stored-city', storedCity);
    }
  }
});
...
const insertOptions = (data, elementSelect, sortKeyName) => {
  data
    ...
    // Establecer el valor almacenado para la ciudad si existe
  if (elementSelect.getAttribute('name') === 'city') {
    const storedCity = elementSelect.getAttribute('data-stored-city');
    if (storedCity) {
      elementSelect.value = storedCity;
    }
  }
};
